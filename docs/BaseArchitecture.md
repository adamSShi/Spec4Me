...
| 類型 | 檔案 | 能做 | 不建議 / 不該做 |
|------|------|------|----------------|
| UI | `.vue` | 畫面、互動 | 商業邏輯、流程判斷 |
| 行為 | `useXxx.ts` | 功能流程、商業邏輯、驗證、API orchestration、狀態管理 | 高複雜度、可獨立存在的核心演算法 |
| 狀態 | `*.store.ts` | 狀態存取、單純衍生 getter | 流程判斷、API 呼叫 |
| 規則 | `*.ts` class / service | 複雜演算法、可獨立測試的 domain 行為 | Vue 依賴、流程調度 |
| 契約 | `*.ts` type / interface | 資料結構定義 | 行為、邏輯 |
